...
import { Data, fromHex, toHex } from 'lucid-cardano'
...
const Raffle = () => {
  ...
  const submit = async () => {
    if (formData.selectedNFTs.length === 0) {
      ...
      // @ts-ignore
      let _raffleId = toHex(formData.title + "-" + floor(formData.minTicket) + "/" + floor(formData.maxTicket)) + posix;
      ...
      let datumRaffle = Data.to(
        [
          ...
          toHex(utf8Encode.encode(formData.title + "-" + floor(formData.minTicket) + "/" + floor(formData.maxTicket))),
