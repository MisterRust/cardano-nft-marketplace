...
const SpecNFT = () => {
  ...
  const submitOffer = async (price: number, policyId: string, asset: string, tokenType: string, nftAssets: string[]) => {
    const response = await makeAnOfferForNFT(myWalletAddress, lucid, price, policyId, asset, tokenType, nftAssets)
    if (response) {
      // close modal for buying nft(s)
  ...
  const getNFTData = async (assets: string[]) => {
    try {
      ...
      for (const asset of assets) {
        ...
        const object = VERIFIED_COLLECTIONS.find((item) => item.policyId === data.policy_id);
        arr.push({
          asset: data.asset,
          name: data.onchain_metadata.name,
          image: data.onchain_metadata.image,
          policyId: data.policy_id,
          description: object ? object.description : 'MINERS MINERS MINERS...',
        });
      }
      setNFTData(arr);
    } catch (error) {
          ...
          <>
            ...
            >
              ...
              <FlexBox direction='column' alignItems='center' justifyContent='center'>
                <ComingSoonTableHeader className='mx-auto'>
                  <div>TRADE TYPE</div>
                  <div>PRICE</div>
                  {
                    !isMobile &&
