...
interface Props {
  ...
  submitSellNFT: (assets: string[], price: number) => Promise<void>;
}
      ...
      <ModalBody>
        ...
        <FlexBox marginTop='32px' paddingLeft='25px' paddingRight='25px' direction='column'>
          <FlexBox justifyContent='start' gap='28px' alignItems='center'>
            <CustomImage
              image={nftData && getExactImageFormat(nftData.image)}
              width='100px'
              height='100px'
              borderRadius='2.4px'
            />
          ...
          <FlexBox marginTop='56px'>
            <CustomButton
              ...
              width='286px'
              height='48px'
              disabled={!inputPrice || inputPrice < 3 || !nftData || myWalletAddress === undefined}
              onClick={() => {
                submitSellNFT([nftData.asset], inputPrice)
              }}
            />
          </FlexBox>
        </FlexBox>
